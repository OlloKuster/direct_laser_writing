...
def f2bin_smooth(rho, alpha, resolution, f2bin):
    ...
    rho_grad_norm2 = (rho_grad[0] / dx) ** 2 + (rho_grad[1] / dy) ** 2 + (rho_grad[1] / dz)*2
...
def ssp_robust_proj_jax_f(alphas, beta, resolution):
    ...
    return (lambda x: (f2bin_smooth(x, alphas[0], resolution, f2bin_eroded),
                       f2bin_smooth(x, alphas[1], resolution, f2bin),
                       f2bin_smooth(x, alphas[2], resolution, f2bin_dilated)))
