...
def frequency_filter_regular_intermediate_plot():
    """
    ...
    def plotter(rho_init, rho_final, cur_eps, projection, i):
        ...
        plt.savefig(f"problems/frequency_filter/plots/progression/rho_{i:03d}.png")
        plt.close()
        ...
        p.remove_scalar_bar()
        p.camera.zoom(1.3)
        p.show(screenshot=f"problems/frequency_filter/plots/progression/eps_{i:03d}.png")
        p.close()

        pv.plot(np.array(cur_eps), off_screen=True, screenshot=f"problems/frequency_filter/plots/progression/eps_density_{i:03d}.png", cmap='binary')

    return plotter


def frequency_filter_regular_final_plot():
    """
    ...
    def plotter(extent, rho_0=None, rho_precomp=None, loss_hist=None, beta=None, em_loss_hist=None, grads=None, eps=None, E=None, run_id=0,
        ...
        if loss_hist is not None:
            ...
            # plt.yscale('log')
            plt.savefig(
                f"problems/frequency_filter/plots/loss_{run_id}_{beta}.png")
            plt.close()

        if beta is not None:
            plt.plot(em_loss_hist)
            plt.savefig(
                f"problems/frequency_filter/plots/em_loss_{run_id}_{beta}.png")
            plt.close()

        if grads is not None:
            plt.plot(grads)
            plt.savefig(
                f"problems/frequency_filter/plots/grads_{run_id}_{beta}.png")
        ...
        if E is not None:
            ...
            plt.imshow(np.abs(E[0][0][E[0][0].shape[0] // 2].T), origin='lower', cmap="magma", alpha=0.8,
                       extent=(0, extent[0], 0, extent[1]))
            plt.xlabel(r"y ($\mathrm{\mu}$m)", fontsize=12)
            plt.ylabel(r"z ($\mathrm{\mu}$m)", fontsize=12)
            plt.savefig(
                f"problems/frequency_filter/plots/eps_and_e_trans_{run_id}_{beta}.png")
            plt.close()
            plt.imshow(eps[eps.shape[0] // 2].T, origin='lower', cmap='binary',
                       extent=(0, extent[0], 0, extent[1]))
            plt.imshow(np.abs(E[1][0][E[1][0].shape[0] // 2].T), origin='lower', cmap="magma", alpha=0.8,
                       extent=(0, extent[0], 0, extent[1]))
            plt.xlabel(r"y ($\mathrm{\mu}$m)", fontsize=12)
            plt.ylabel(r"z ($\mathrm{\mu}$m)", fontsize=12)
            plt.savefig(
                f"problems/frequency_filter/plots/eps_and_e_refl_{run_id}_{beta}.png")
            plt.close()
        ...
        if eps is not None:
            ...
            plt.ylabel(r"z ($\mathrm{\mu}$m)", fontsize=12)
            plt.savefig(
                f"problems/frequency_filter/plots/eps_{run_id}_{beta}.png")
            plt.close()

        if save:
            with h5py.File(
                    f"problems/frequency_filter/plots/data_{run_id}_{beta}.h5",
                    'w') as f:
...
def frequency_filter_robust_intermediate_plot():
    """
    ...
    def plotter(rho_init, rho_final, cur_eps, projection, i):
        ...
        pv.plot(np.array(cur_eps), off_screen=True,
                screenshot=f"problems/frequency_filter/plots/progression/eps_density_{i:03d}.png", cmap='binary')

    return plotter


def frequency_filter_robust_final_plot():
    """
    ...
    def plotter(extent, rho_0=None, rho_precomp=None, loss_hist=None, beta=None, em_loss_hist=None, grads=None, eps=None, E=None, run_id=0,
        ...
        if E is not None:
            ...
            plt.savefig(
                f"problems/frequency_filter/plots/eps_and_e_{run_id}_{beta}.png")
            plt.close()
