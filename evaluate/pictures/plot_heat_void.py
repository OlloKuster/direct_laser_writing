...
heat_sinks_void = jnp.zeros((rho_bin.shape[1] + 1,
...
heat_sinks_void = heat_sinks_void.at[:, -1].set(True)
heat_sinks_void = heat_sinks_void.at[..., -1].set(True)
kappa_r_matter = f2param(1-rho_bin_heat, (1e-5, 1))
fem_matter = FEA2D_T(heat_sinks_void)
src_matter = jnp.pad(rho_bin_heat, [(0, 1), (0, 1)], mode='constant', constant_values=0)
T_matter = fem_matter.temperature(kappa_r_matter, src_matter).reshape(heat_sinks_void.shape)
...
plt.imshow(1-rho_bin[rho_bin.shape[0]//2-5].T, origin='lower', cmap='binary')
plt.imshow(T_matter.T, origin='lower', cmap='hot', alpha=0.8)
plt.axis('off')
plt.savefig("plots/dlw_filter/temp_void.png")
plt.close()