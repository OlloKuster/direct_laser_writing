...
with h5py.File("../data/feature_size_check_old/data_5_inf.h5") as f:
    grp = f["lens_3d"]
    rho_00 = grp["rho"][:]
    E = grp["E"][:]

dlw_filter = filter_loader("dose_conv", 14, 2*0.003)
...
#
#
# E, eps = em_simulation(jnp.array(rho_bin), currents, resolution)
...
plt.imshow(E[E.shape[0]//2-5].T, origin='lower', cmap='RdBu')
plt.axis('off')
plt.show()
