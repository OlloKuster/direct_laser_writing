...
from problems.frequency_filter.simulation._objective_loader import objective_loader
from problems.frequency_filter.simulation.config_structure import ConfigSim
from problems.frequency_filter.simulation.simulation import em_simulation
...
def run(resolution, betas, setting: dict, loss_hist, em_loss_hist, opt, max_evals, load=None, eval=False, run_id=0):
    ...
    for i in range(len(betas)):
        ...
        if type(rho_opt_proj) is tuple:
            ...
            E_trans, eps = em_simulation(jnp.array(rho_opt_proj), ConfigSim.wavelengths[0], currents, resolution)
            E_refl, eps = em_simulation(jnp.array(rho_opt_proj), ConfigSim.wavelengths[1], currents, resolution)
