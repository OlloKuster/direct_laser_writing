...
from problems.frequency_filter.simulation.simulation import em_simulation, heat_simulation
...
def objective_em_f(currents, resolution, init_value):
    ...
    def objective_em(rho):
        ...
        """
        E_trans, eps = em_simulation(rho, ConfigSim.wavelengths[0], currents, resolution)
        trans = E_trans[0][
            :, :, int(jnp.ceil(ConfigSim.location_trans_monitor * resolution))]
        E_refl, eps = em_simulation(rho, ConfigSim.wavelengths[1], currents, resolution)
        refl = E_refl[0][
            :, :, int(jnp.ceil(ConfigSim.location_trans_monitor * resolution))]

        jprint("--------------")
        jprint("trans: {trans}", trans=jnp.sum(jnp.abs(trans)))
        jprint("refl: {refl}", refl=jnp.sum(jnp.abs(refl)))
        jprint("--------------")
        return jnp.sum(jnp.abs(trans) - jnp.abs(refl)), ((jnp.sum(jnp.abs(trans)), jnp.sum(jnp.abs(refl))), eps)
...
def objective_robust_em_heat_f(currents, resolution, init_values):
    ...
    def objective_softplus(rho):
        ...
        """
        v_lens, eps = objective_em(rho)
        v_heat_m, v_heat_v = objective_heat(rho, resolution)
    ...
    def objective_robust_softplus(rhos):
        ...
        return fom_min, ((v_lens_eroded, v_lens_normal, v_lens_dilated), eps_normal)
