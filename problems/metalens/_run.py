...
def run(resolution, betas):
    ...
    objective_0 = objective_loader("em_only", currents, resolution, 1)
    init_val = objective_0(rho_0)

    loss_hist = []

    for beta in betas:
        objective = objective_loader("em_only", currents, resolution, init_val)
        ...
        rho_opt_filtered = filter(torch.tensor(rho_0, device='cuda'))  # todo: build the conversions into the filter/projection filter themselves
